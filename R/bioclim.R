...
function(tmin=NULL, tmax=NULL, prec=NULL, tmean=NULL, vois=1:19, cov=FALSE, t.as.int=TRUE, period="month") {
	...
	if (any(vois == 4)) {
		if (cov) { #calculate it as anuclim does (cov)
			out[,4] = (apply(tmean,1,function(x) { return(sd(x,na.rm=T)) }) / (out[,1]+273.15)) * 100
		} else { #calculate it as worldclim does (sd)
	...
	if (any(vois %in% c(10:11,18:19))) {
		...
		if (any(vois %in% 18:19)) {
			...
			if (any(vois == 18)) { 
				tt = NULL; for (ii in 1:nrow(tt2)) { tt = c(tt,which(tt1[ii,]==out[ii,10])) }
				out[,18] = tt2[cbind(1:nrow(tt2),tt)]
			}
			#precipitation of the coolest quarter -- 19
			if (any(vois == 19)) { 
				tt = NULL; for (ii in 1:nrow(tt2)) { tt = c(tt,which(tt1[ii,]==out[ii,11])) }
	...
	if (any(vois %in% c(8:9,16:17))) {
		...
		if (any(vois %in% 8:9)) {
			...
			if (any(vois == 18)) { 
				tt = NULL; for (ii in 1:nrow(tt2)) { tt = c(tt,which(tt1[ii,]==out[ii,16])) }
				out[,18] = tt2[cbind(1:nrow(tt2),tt)]
			}
			#mean temperature of the driest quarter -- 11
			if (any(vois == 19)) { 
				tt = NULL; for (ii in 1:nrow(tt2)) { tt = c(tt,which(tt1[ii,]==out[ii,17])) }
				out[,18] = tt2[cbind(1:nrow(tt2),tt)]
			}
			rm(tt2);rm(tt)
		}
